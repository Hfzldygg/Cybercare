...
import { MessageSquare, Check, Calendar, Send, Clock, UserCheck } from "lucide-react";
  ...
  return (
    <div id="guru-counseling-panel" className="space-y-6 animate-fade-in text-slate-805">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-205 shadow-md space-y-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-205 shadow-md flex flex-col h-[520px] overflow-hidden">
          ...
          {activeCounselingRoom ? (
            <>
              ...
              <div className="px-6 py-4 bg-slate-50 border-b border-slate-150 flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-xs text-slate-400 uppercase tracking-lighter">Nama Siswa / Sesi Terpilih</h4>
                  <h3 className="font-extrabold text-sm text-slate-800 truncate mt-0.5">{activeCounselingRoom.siswaNama} ({activeCounselingRoom.siswaKelas})</h3>
                  <p className="text-[10.5px] text-slate-505 mt-0.5">Topik: <span className="font-semibold text-indigo-600">{activeCounselingRoom.topik}</span></p>
                </div>
                <div className="flex items-center gap-1.5 bg-indigo-50 border border-indigo-100/50 px-3 py-1 rounded-full shrink-0">
                  <UserCheck className="w-3.5 h-3.5 text-indigo-600" />
                  <span className="text-[9.5px] font-bold text-indigo-600">{activeCounselingRoom.status}</span>
                </div>
              </div>
